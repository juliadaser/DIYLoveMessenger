...
void loop() {
  ...
  if (Firebase.RTDB.setInt(&fbdo, "/user_two", buttonSend)) {
    //Serial.println("");
  ...
  // downloading the other person's data
  if (Firebase.RTDB.getInt(&fbdo, "/user_one")) {
    if (fbdo.dataType() == "int") {
      ledStatus = fbdo.intData();
      if (ledStatus != 0) {
        // light up our LED in the same color as the other person
